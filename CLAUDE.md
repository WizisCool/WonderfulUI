...
- **Vue SFC and store-action tests** use **vitest** + `@vue/test-utils` + `happy-dom` (`.component.test.ts`), kept separate from the bun:test runner. Bun cannot process `.vue` SFC imports natively; vitest handles them through `@vitejs/plugin-vue`. Component tests inject `createTestingPinia`; stateful store tests may use a real `createPinia` plus mocked adapter boundaries. `bun run typecheck` is `vue-tsc --noEmit` so templates are part of the type gate.
...
- Account preference optimism is revision-scoped: do not roll a failed order/name write over a newer library load, and merge a successful late write into the current account collection so newly discovered accounts are preserved.
- All Tauri `invoke()` calls go through **Pinia store actions** (8 stores: `account`, `filter`, `detail`, `player`, `settings`, `share`, `update`, `ui`). Components never call `invoke()` directly.
- GUI calls `scan_all()` / `scrape_library(mode?: "incremental" | "full")` to refresh the SQLite library, then receives parsed match JSON from the library view. Startup uses incremental scan; the match-list header refresh button uses the user's saved scan mode from the settings modal (`增量扫描` / `全量扫描`). The bulk payload is **rounds-stripped** (see `strip_match_rounds`) to keep IPC small. Scan/probe commands intentionally accept no renderer-supplied directory or audit trigger: only the fixed absolute ACLOS WonderfulDb path is readable, persisted triggers are backend-derived, and missing profile environment fails closed.
- Round / clip / event data is fetched on demand via the `get_match_rounds(openid, match_id)` Tauri command, which accepts only a known bounded account ID and a bounded single-line match ID before reading the full match JSON from SQLite.
- Detail round loads are selection-versioned in `stores/detail.ts`: late responses must match both `matches_id` and the current selection version, failures become an explicit retry state without exposing backend details, and library refreshes reselect same-id match objects by identity. Never mutate a replacement match from an older request or rethrow a fire-and-forget detail load.
...
- **Settings charts use Apache ECharts via `vue-echarts`** (`packages/gui/src/charts/register.ts` + `AccountShareChart.vue`). Register only chart/components used by visible shipping UI (currently Pie + Tooltip + Legend + CanvasRenderer); speculative Bar/Line/Grid/Dataset/DataZoom/Title registration defeats tree-shaking. Build pure options in `library-stats.ts` (or future `charts/*-option.ts`); render with `<VChart autoresize />`. **Do not** call `echarts.init` / module-level chart singletons — Vue owns mount/unmount (tab `v-if` and modal close). Center totals stay as sibling DOM overlays, not series labels. `SettingsView` stays behind `defineAsyncComponent` and `settings.isOpen` in App plus a dynamic router import so ECharts never rejoins the startup chunk.
...
- Never open ACLOS LevelDB in place: `rusty-leveldb` may write `LOCK`/logs even with `create_if_missing=false`. Identity lookup must copy regular files to app-owned temporary storage, skip the live `LOCK`, open only the copy, then clean it up. ACLOS remains read-only.
- Legacy asset-cache downloads must reject both declared and streamed zero-byte bodies, enforce the 16 MiB cap, and revalidate the published regular file before updating SQLite. Never turn an empty HTTP 200 response into a successful cache entry.
- Legacy `cache_assets` IPC must reject more than 256 entries or oversized URL/kind strings before worker creation. Keep the batch command fallible even though canonical bundled assets normally produce zero work; an exposed compatibility boundary cannot permit unbounded network queues.
...
- ACLOS probe/scan IPC must not accept a renderer-supplied directory. The product has no custom source picker; resolve only the fixed absolute profile `AppData\Roaming\ACLOS\WonderfulDb` path and fail closed if the profile environment is absent or relative.
- Every SQLite connection must set the 5 s busy timeout before WAL PRAGMAs/migration. Tauri commands use separate connections and normal scan/preference/cache overlap must wait for a short writer handoff instead of surfacing the default 0 ms `SQLITE_BUSY` failure.
- App-log status, compaction, and append operations share `LOG_LOCK`. Background scraper/share/cache threads and IPC may log concurrently; never read-tail/rewrite the log outside that lock or compaction can discard an overlapping append.
- All log callers, including renderer `log_event`, pass through the bottom-level 64-character scope and 8192-character single-line message bounds before append. Keep the explicit truncation marker; the 1 MiB compactor is not a substitute for bounding one hostile or accidental log call.
- **`scan_shell` background scrape race:** `scan_shell` returns immediately with the existing SQLite library view, then spawns a background thread to refresh from WonderfulDb. On first launch the local library is empty, so the frontend must NOT reveal the main UI until the background refresh settles. Subscribe to the scan-specific `wui://startup_refresh_finished` event **before** calling `scan_shell`; unlike `wui://scrape_summary`, it is emitted for success, partial-account degradation, degraded fallback, and fatal failure. Tauri events are not buffered. `runBoot()` waits up to 30 s, but timeout releases only the overlay and must not unregister or settle the real terminal session. Late completion performs a revision-guarded SQLite reload; pending retries reuse the same session so one unkeyed terminal event cannot complete the wrong startup job.
...
- Account sorting uses the shared stable `applyAccountOrder` rule. A partial submitted order moves those IDs first but must retain every unmentioned/new account in prior relative order; persist the resulting complete ID list so frontend and SQLite order semantics cannot diverge.
- Account preference IPC accepts only existing SQLite account IDs. Reject duplicate/oversized order payloads before the transaction, and keep non-empty custom names to at most 64 Unicode characters with no control characters; never let renderer input create orphan preference rows.
- Settings log/stat reads share their in-flight Promise. A full scan must call `refreshLibraryStats()`, which waits out a pre-scan read and then issues one post-scan read; simply returning while `statsLoading` is true leaves stale statistics on screen.
...
- Toast requests are events, not boolean state: every `ui.showToast()` increments `toastSeq`, including repeated text while another toast is visible. ToastHost consumes the sequence with `flush: 'sync'` (same-tick calls cannot coalesce) and `immediate` recovery (boot errors may precede host mount), then owns and clears every per-item removal timer. Do not restore a `toastVisible` watcher that drops notifications.
- Filter/local preference persistence is best-effort. In particular, refresh scan mode must safely fall back to `incremental` when `localStorage` is absent, corrupt, or throws, and a write failure must not reject the user's in-memory mode change or break Pinia initialization.
...
- Icons come from @iconify/vue (Phosphor set), via the shared `WIcon.vue` wrapper (`packages/gui/src/components/common/WIcon.vue`). Every referenced `ph:*` literal must exist in the generated offline subset; `offline-icons.test.ts` enforces this. Do not use Unicode symbol glyphs for controls or rely on Iconify CDN fallback.
...
- **Modal keyboard ownership follows z-index, not listener order**: `utils/modal-layer.ts` defines share (1500) → update (1400) → settings (1300) → player (1200) → event list (1100). Every document-level modal key handler must gate on `ownsTopModalLayer()`, and `nextCloseableLayer` must keep the same order. This prevents a higher dialog's Escape/Space/arrows from also acting on the player or event list underneath.
- **Modal focus uses one boundary helper**: settings/update/share/event roots call `useDialogFocus()`; PlayerHost calls `trapDialogTab()`. Dialog roots are programmatically focusable with `tabindex="-1"`, restore the opener on close, and prevent Tab only when wrapping at a boundary. Never restore `@keydown.tab.prevent` on the player or another modal—it blocks native movement between interior controls.
...
- **Buffering timer lifecycle**: `waiting` while `playing` starts a 300 ms debounce timer before entering `buffering`. The timer MUST be cleared in `onCanPlay`, `onPlay`, `onPause`, `onEnded`, `onError`, `doClose`, and the playback-session reset/close watcher. Failure to clear the timer causes stale `buffering` state that locks the loading overlay.
- **Player session identity**: every `player.open()` increments `sessionSeq`, including the same video reopened at another seek. Reset seek, buffering, FPS measurement, frame hold, context/share, and screenshot UI per sequence. FPS callbacks and async screenshot work must validate the captured sequence/video/path before committing or resuming. Normalize letter hotkeys to lowercase so Shift+J/L performs the documented five-frame step.
- Windows Media Foundation frame buffers are length-bounded before every unsafe row copy. Prefer `IMF2DBuffer2::Lock2DSize` (handles positive/negative pitch with buffer start/length), use `ContiguousCopyTo` for older 2D buffers, and validate plain `IMFMediaBuffer::cur_len`. Keep checked width/height multiplication, the 512 MiB decoded-frame cap, and checked `u64` milliseconds → signed 100 ns conversion so renderer input cannot wrap to frame zero.
...
- **Controls click propagation**: `.player-controls` div has `@click.stop` to prevent clicks on the controls bar (volume slider, progress track, time text, gaps) from bubbling to `.player-stage`'s `togglePlay`. Individual buttons also have `@click.stop`.
- **Player volume boundary**: persisted `0` is a valid volume, while malformed/non-finite values fall back or are ignored. Clamp every accepted value to 0–100 before writing `HTMLMediaElement.volume`; ignore zero-width slider geometry. The volume track is a keyboard-accessible slider (arrows ±5, Home 0, End 100).
- **Frame stepper**: pause-only top-left dock (`film-strip` +「逐帧」, expands to mono `−1`/`+1`). Default collapsed; expand uses `0fr→1fr` grid. Hold ±1 for continuous step paced by `seeked` (≤15 Hz, no seek flood). No skip/caret icons.
...
- **OS-spawn fire-and-forget**: `reveal_in_explorer` and `reveal_logs_dir` in `src-tauri/src/lib.rs` must use `Command::spawn()` with `Stdio::null()`, never `.status()`. The previous `.status()` call blocked the Tauri command thread until the outer `cmd.exe` / `explorer.exe` finished initializing (Explorer + DWM handshake ≈ 300–800 ms), which surfaced as a visible UI freeze on right-click menu close. `explorer.exe` exits with code 0 or 1 in `/select` mode regardless of success; treat successful spawn after the command-specific path gate as success.
- **Library video IPC boundary**: `play_video`, `reveal_in_explorer`, `start_share_server`, and `capture_video_frame` accept only an exact SQLite `videos.path` with a supported video extension that is still a non-symlink regular file. Keep this in the shared validator and extend `local_video_commands_require_a_registered_supported_file` when the rule changes; never replace it with a frontend-only check or broad filesystem scope.
- **`play_video` uses native Win32**: `src-tauri/src/os_shell.rs::shell_open` calls `ShellExecuteW` directly via the `windows` crate (0.61, deduped with Tauri's transitive dep). Do **not** route this through `cmd /c start ""` — the `cmd.exe` process layer + `start` builtin lookup is exactly the ~300–800 ms right-click menu lag we already fixed. `ShellExecuteW` is in-process and the same API Explorer uses.
- **Tauri asset/CSP boundary**: production CSP is explicit; `devCsp: null` is intentionally dev-only. Asset protocol scope must never return to `"**"`: app cache has the one static `$LOCALDATA/wonderful-ui/assets/**` scope, while current SQLite video/poster regular files are allowed dynamically. Screenshot Save As relies on the dialog's single-path grant with `fs:allow-write-file`; do not restore global `fs:scope` or `fs:allow-read-file`.
- **Never use Windows `IDataTransferManagerInterop` / UWP Share Sheet for "share to other app" flows.** The path looks clean in docs (`RoGetActivationFactory` cast to `IDataTransferManagerInterop`, AUMID, `run_on_main_thread` for STA), but in practice we hit: (1) `0x80040154 REGDB_E_CLASSNOTREG` because the CLSID looks COM-registered but is actually a WinRT runtime class; (2) Win11 "重试 我们无法为您显示所有共享方法" overlay that hides every third-party target (LocalSend, OneDrive, etc.); (3) AUMID must be set on the **main thread** during `app.setup()` and the entire call chain must run STA. **We abandoned this in 2026-06-23** in favor of our own "快传" HTTP+QR flow (`src-tauri/src/share_server.rs` + `ShareModal.vue`) — no UWP, no AUMID, zero third-party protocol implementation. The obsolete `SharePlatform` registry/popover scaffolding was removed because production never called it; do not recreate placeholder architecture before a second real share target exists.
...
- **Share lifecycle is session-scoped and cancellable.** Every frontend start allocates a canonical UUID v4 `sessionId` (normally `crypto.randomUUID()`; older runtimes must still format random bytes as v4, never fall back to a counter string). `start_share_server`, `stop_share_server`, `ShareServerInfo`, `wui://share_downloaded`, and `wui://share_server_stopped` carry it. IPC accepts only canonical UUID v4 values. The store invalidates its session synchronously on close, cleans a late start result with that exact ID, and ignores old-session events. Rust records a stop that arrives before publication and checks it atomically before replacing the current server, but this cancellation table must stay bounded (60 s TTL, 128 entries) so stale stops cannot leak memory. `ShareModal` must check its `disposed` flag after each async `listen()` before starting the server, and immediately unsubscribe a listener that resolves after unmount.
- **Share stop reasons are semantic.** Manual/replacement shutdown emits `stopped`; the three-minute idle branch emits `idle_timeout`; failures emit `error`. Do not collapse all successful loop exits back to `stopped`, because the modal uses `idle_timeout` to explain and close an expired share.
- **Dynamic progress accessibility:** update and share progress tracks use `role="progressbar"`; omit `aria-valuenow` for indeterminate states, expose it only for a known percentage/completion, and provide `aria-valuetext`. Share download status is a polite atomic live region, the QR button has an explicit copy label, and raw native paths/network errors remain local-log only.
- **Updater modal terminal state:** both a retry check and the install-time fresh `check()` may report no update. Those branches must set `uptodate` and close `modalOpen` together; `UpdateModal` has no visible `uptodate` surface, so leaving it open creates an empty, non-closeable backdrop.
- **LAN address is a start-time invariant**: `lan_ip::detect_lan_ipv4()` uses the kernel-selected default route, then operational-interface enumeration as a real fallback. `start_server` must capture that one IPv4 and pass it into `run_server`; never redetect inside the server thread, because a multi-adapter route change can make Host validation reject the QR URL already shown to the user. No address means an explicit startup error, not a `127.0.0.1` QR code.
