...
- **CSS GPU compositing**: `filter: brightness()` replaced with `::after` overlays, `backdrop-filter: blur()` removed, progress bar uses `transform: scaleX()` / `translateX()` instead of `width`/`left`, ECharts uses `renderer: 'canvas'`. All component styles use `<style scoped>`; only CSS custom properties, reset, fonts, and shared utilities remain in `assets/style.css`. **ECharts watch must use `{ flush: 'post', immediate: true }` + `nextTick`** — the chart DOM is behind `v-else`, so the default `flush: 'pre'` fires before Vue re-renders and `chartRef` is still `null`. **Guard against zero-size containers** — ECharts `init()` on a 0×0 host creates a permanent 0×0 canvas; defer via `requestAnimationFrame` until real dimensions are available.
...
- **Player right-click menu close contract**: The player right-click menu (`PlayerHost.vue` `openContextMenu` / `closeCtxMenu`) registers a document-level listener on `mousedown` in capture phase, filtered by `e.button === 0`. Do **not** switch this to a `click` listener — a right-click fires `mousedown → mouseup → click (button=2) → contextmenu` in that order; a `click` handler would see the right-click's own synthesized `click` *after* the menu opened and immediately close it. The current mousedown+button-0 contract ignores the opening right-click and only reacts to the next intentional left-click. The exit animation uses `v-show` + an `is-closing` class with a 200 ms safety timeout, and the `animationend` handler is filtered by `animationName === 'player-ctxmenu-out'` so bubbled descendant animations can't finish the close prematurely.
