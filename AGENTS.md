...
- **Event markers use DOM rendering only.** Canvas mode was disabled (see `CANVAS_MARKER_THRESHOLD` in `player-event-markers.ts`) because the 4px-tall canvas clipped all dot rendering outside its bounds and produced visual inconsistencies vs DOM markers. If canvas mode is re-enabled, the canvas CSS must be extended (e.g. `top: -45px; bottom: -30px`) and `renderCanvasMarkers` must derive `trackY` from CSS `top` offset, not `rect.height / 2`.
- Tooltips and floating overlays use `packages/gui/src/composables/useFloating.ts` (`@floating-ui/dom`), a shared positioning utility with cursor-aware alignment and edge flips. The global `.tooltip` element lives on `document.body` with a `.floating-arrow` indicator. **Tooltip wiring** is in `App.vue` via delegated `mouseover`/`mouseout`/`mousemove` listeners that read `[data-tip]` attributes and invoke the `useTooltip()` composable. Components set `data-tip` on their DOM elements; no custom directive or per-component binding is needed.
