...
type ScrapeMode = 'incremental' | 'full';

const REFRESH_SCAN_MODE_KEY = 'wui:library.refreshScanMode';

function scanModeLabel(mode: ScrapeMode): string {
  return mode === 'full' ? '全量扫描' : '增量扫描';
}

function loadRefreshScanMode(): ScrapeMode {
  return localStorage.getItem(REFRESH_SCAN_MODE_KEY) === 'full' ? 'full' : 'incremental';
}

function saveRefreshScanMode(mode: ScrapeMode) {
...
function settingsModal(scraping: boolean, scanMode: ScrapeMode, closing = false): HTMLElement {
