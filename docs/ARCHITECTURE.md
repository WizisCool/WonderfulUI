...
- **In-app player media:** custom URI scheme `wui-media` (`src-tauri/src/media_protocol.rs`) serves local highlight files with `Access-Control-Allow-Origin: *` and HTTP Range. Frontend uses `convertVideoSrc` + `crossOrigin="anonymous"` so progressive play stays fast and canvas export is not tainted. Default `asset` protocol remains for images.
