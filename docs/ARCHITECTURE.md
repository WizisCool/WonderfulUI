...
- **Screenshot save** (player context menu): `tauri-plugin-dialog` (Save As) + `tauri-plugin-fs` (`writeFile`). Frame capture stays in the frontend (`canvas.drawImage` on `<video>`). Clipboard uses browser `ClipboardItem` (image/png).
- **In-app player media:** playback uses Tauri **asset** protocol (`convertFileSrc`) for progressive Range streaming. Custom `wui-media` exists with capped chunks + CORS but is **not** used for the player (full-body `Vec` responses caused open freezes). Screenshots use canvas draw with a lazy same-origin blob clone when the asset URL taints the canvas.
