...
- **Screenshot busy UI:** copy → dim + progress → capture → toast. Save → **display freeze**: paint current frame to a covering canvas (`paintVideoFrameToCanvas`; export-taint OK for display), hide live `<video>` pixels, best-effort `pause()` + seek under the freeze, then Save As → progress → native capture → toast → clear freeze + **resume if was playing**. Cancel restores play. Hold also blocks canplay/onPlay/togglePlay/Space/J/K/L and runs a pause watchdog.
