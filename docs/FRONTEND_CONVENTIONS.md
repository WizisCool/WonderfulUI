...
- **Tainted canvas / fast screenshot:** `asset.localhost` taints canvas. On open: start with `convertFileSrc` (fast first paint), **prefetch** file into a cached same-origin `blob:` URL, then **hot-swap** the live `<video>` to that blob after `canplay` so capture is one `drawImage`+`toBlob` (~ms). If still tainted, reuse a hidden clone on the cached blob (seek only, no re-read). Release cache on close / clip change. Needs `fs:allow-read-file` when fetch fails.
