...
- **Screenshot busy UI:** copy → dim + progress → capture → toast. Save → **immediately hard-hold pause + seek to frozen frame** (before Save As) → dialog → re-assert pause → dim+progress → capture+write → toast → **resume if was playing**. Cancel dialog restores prior play. Hold uses `screenshotPlaybackHold` + 32ms pause watchdog; blocks canplay/onPlay/togglePlay/Space/J/K/L.
