...
- **Screenshot busy UI:** copy → close menu → dim player shell + indeterminate accent progress (`player-screenshot-overlay`) → capture → toast → clear. Save → close menu → system Save As first → on path chosen, **hard-hold pause** on frozen frame (`screenshotPlaybackHold` blocks canplay/onPlay auto-resume after seek), wait `seeked`, dim+progress → capture+write → toast → **resume if was playing**. Freeze `timeMs` at menu action (before dialog) so the frame matches the click.
