...
- **Player video URL:** always `convertFileSrc` (Tauri **asset** protocol) for progressive Range streaming. **Do not** hot-swap live `src` to `blob:`, and **do not** play via a custom scheme that buffers whole files into memory (UI freeze on open). Screenshots: live draw often taints on `asset.localhost` → lazy same-origin blob clone only when the user captures (no open-time prefetch).
