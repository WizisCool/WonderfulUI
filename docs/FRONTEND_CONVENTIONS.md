...
- **Screenshot busy UI:** copy → dim + progress → capture → toast. Save → paint freeze frame onto **modal-level** canvas layer (z-index 30, outside `.player-stage` — WebView2 HW video punches through in-stage siblings) → collapse live `<video>` with opacity+1×1px (not only `visibility:hidden`) → **flush nextTick+2×rAF before Save As** → pin scrubber / ignore `timeupdate` → progress overlay → native capture → clear freeze + resume if was playing.
