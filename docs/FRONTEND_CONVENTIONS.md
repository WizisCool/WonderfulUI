...
- **Screenshot busy UI:** copy → dim + progress → capture → toast. Save → **display freeze** (`paintVideoFrameToCanvas` + hide video) → **`await nextTick` + 2× rAF before native Save As** (or freeze never paints) → pin `currentTime` / ignore `timeupdate` while held → progress → capture → toast → clear freeze + resume if was playing. Cancel restores play.
