...
- **Player video URL:** use `convertVideoSrc` (`wui-media` custom protocol: CORS + HTTP Range) with `<video crossorigin="anonymous">`. Live `drawImage`+`toBlob` is export-safe without blob prefetch/hot-swap. **Do not** hot-swap live `src` to `blob:` — that reloads the clip and kills first-play. Posters/map/hero images stay on default `convertFileSrc` (asset). Taint fallback: lazy same-origin blob clone only when live draw fails (browser-debug / unexpected path).
